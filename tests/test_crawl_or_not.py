...
def test_no_conditions():
    ...
    result2 = _crawl_or_not(f'{url}?one=1')
    result3 = _crawl_or_not(f'{url}/one/two/three')
    result4 = _crawl_or_not(f'{url}/one/two?hello=yes&1=2')
    assert all([result1, result2, result3, result4])


def test_exclude_url_params_true():
    result = _crawl_or_not(f'{url}?1=1', exclude_url_params=True)
    assert not result


def test_reject_right_param():
    result = _crawl_or_not(f'{url}?one=1', exclude_url_params=['one', 'two'])
    assert not result


def test_dont_reject_param():
    result = _crawl_or_not(f'{url}?hello=1', exclude_url_params=['one', 'two'])
    assert result


def test_include_right_param():
    result1 = _crawl_or_not(f'{url}?one=1&two=2', include_url_params=['one'])
    result2 = _crawl_or_not(
        f'{url}?one=1&two=2', include_url_params=['one', 'two']
    )
